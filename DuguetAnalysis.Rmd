...
1. I remove all the contaminants by getting rid of the `Majority.protein.IDs` containing `REV\_` or `CON\_`.
...
From the MaxQuant export table, there are `r input` unique "Majority.protein.IDs". After removing the contaminants, `r input2` remain, after removing the rows with only NAs, there are `r nrow(mat)` unique "Majority.protein.IDs" remaining.
...
Exp <- c(".1", ".2", ".3", ".4", ".5", ".6", ".7")
pdf("Figures/QCTechnicalByReplicates.pdf", useDingbats=FALSE, 11.69, 8.27) # width and height in inches.
for (el in Exp) {
...
col1 <- colorRampPalette(c("darkred", "red", "indianred1", "white", "white", "white", "cornflowerblue", "blue", "darkblue")) 
corrplot.mixed(M, lower = "square", upper = "number", col=col1(600)) 

pdf("Figures/QCTechnicalReplicates.pdf", useDingbats=FALSE, 11.69, 8.27) # width and height in inches.
corrplot.mixed(M, lower = "square", upper = "number", col=col1(600)) 
dev.off()
dev.off()
```
...
#write.table(cbind(Means, "Protein" = row.names(Means)), "OutputTables/Duguet_Means.txt", sep = "\t", row.names = F)
...
pdf("Figures/QCMeanPerCondition.pdf", useDingbats=FALSE, 8.27, 8.27) # width and height in inches.
Exp <- c("Tconv", "Treg")
for (el in Exp) {
  sub <- Means[,grepl(el, dimnames(Means)[[2]])]
  pairs(sub, pch = ".", main = el, upper.panel = panel.cor)
...
pdf("Figures/QCCVs.pdf", useDingbats=FALSE, 8.27, 4) # width and height in inches.
ggplot(melt(CV), aes(x = value)) + geom_histogram() + theme_bw() + ggtitle("histogram of CVs obtained from technical replicates")
dev.off()
```
...
pdf("Figures/QCMeanReplicates.pdf", useDingbats=FALSE, 8.27, 8.27) # width and height in inches.
pairs(Means, pch = ".", upper.panel = panel.cor)
dev.off()
```
...
pdf("Figures/QCCVMeans.pdf", useDingbats=FALSE, 8.27, 4) # width and height in inches.
ggplot(melt(CV), aes(x = value)) + geom_histogram() + theme_bw() + ggtitle("histogram of CVs obtained between means of biological replicates")
dev.off()
```
...
We then calculate the pvalue of two-sided paired t-test. The p-values from proteins that have a minimum of 2-fold change between Treg and Tconv (log2(mean(Treg))-log2(mean(Tconv)) above or equal to 1 or log2(mean(Treg))-log2(mean(Tconv)) under or equat to -1) are adjusted (BH). The fold change (FC) coresponds to log2(mean(LFQ in Treg) / Mean(LFQ in Tconv)). Everything is saved with the raw table in the table `OutputTables/TotalAnalysisTconvTreg.txt`.
...
#write.table(export, "OutputTables/TotalAnalysisTconvTreg.txt", row.names=F, sep = "\t")
```
...
#write.table(signM, "OutputTables/SignificativeHits.txt", row.names = F, sep = "\t")
```
...
The output figure is saved as a .pdf: `Figures/Volcano.pdf`.
...
pdf("Figures/Volcano.pdf", useDingbats=FALSE, 11.69, 8.27) # width and height in inches.
g
dev.off()
```
...
The figures are saved as .pdf in the folder `/Figures`: `BarPlotUp.pdf` and `BarPlotDown.pdf`.
...
pdf("Figures\\BarPlotUp.pdf", useDingbats=FALSE, 11.69, 8.27) # width and height in inches.
...
dev.off()
pdf("Figures\\BarPlotDown.pdf", useDingbats=FALSE, 11.69, 8.27) # width and height in inches.
...
for (k in round(seq(from = 1, to = (nrow(mat1)-(nrow(mat1)/j)+1), by = nrow(mat1)/j), 0))  {
  ...
  na <- paste0("Figures\\MultipleBoxPlotsUp", n, ".pdf")
...
for (k in round(seq(from = 1, to = (nrow(mat1)-(nrow(mat1)/j)+1), by = nrow(mat1)/j), 0))  {
  ...
  na <- paste0("Figures\\MultipleBoxPlotsDown", n, ".pdf")
  pdf(na, useDingbats=FALSE, 8.27, 11.69) # width and height in inches.
  multiplot(plotlist = plots, cols = 4)
  dev.off()
  n <- n+1
...
I save the individual boxplots in `Figures/MultipleBoxplotsUp.pdf` and `Figures/MultipleBoxplotsDown.pdf`.

I create a table for the paper:

```{r, echo = F}
tablePaper <- df[,c(9,10,2,3,4)]
write.table(tablePaper, file = "OutputTables/Table1Paper.txt", sep = "\t", row.names = F)
```
...
This heatmap is ordered in function of the sum of the LFQ intensities in the Treg, decreasing. It is named `Figures/TileMap.pdf`.
...
pdf("Figures\\TileMap.pdf", useDingbats=FALSE, 8.27, 11.69) # width and height in inches (A4 portrait).
grid.arrange(top, bottom, ncol=1, grobs = grobs, heights=c(nrow(dftop), nrow(dfbottom)*1.7))
dev.off()
```

I make the same figure with only the 40 most regulated proteins up and down (corresponds to the proteins in the barcharts). This is called `Figures/TileMapRegulated.pdf`.
...
pdf("Figures\\TileMapRegulated.pdf", useDingbats=FALSE, 8.27, 11.69) # width and height in inches (A4 portrait).
grid.arrange(top, bottom, ncol=1, heights=c(40, 46))
dev.off()
```
