...
The final table is saved as `OutputTables/SignatureFoxp3.txt`.


```{r Expression, echo = F}
tab <- read.csv("OutputTables/TotalAnalysisTconvTreg.csv", stringsAsFactors = F)
...
pdf("Figures//ExpressionCorrelation.pdf", useDingbats=FALSE, 11.69, 8.27) # width and height in inches.
g
dev.off()
...
#write.table(exp, "OutputTables//signatureFoxp3.txt", sep = "\t", row.names = F)
...
The table `OutputTables/signatureFoxp3.txt` contains the initial expression data with an additional column stating if the gene is quantified in our MS dataset.

The table `OutputTables/TableAll.txt` contains our result table with the additional columns of mean in each dataset, including a `mapping` column that is the first Majority protein ID that I use to merge the other datasets to ours (see the following section).
...
pdf("Figures//ProteomicsCorrelation.pdf", useDingbats=FALSE, 11.69, 8.27) # width and height in inches.
g
dev.off()

#write.table(tab11, "OutputTables/TableAll.txt", sep = "\t", row.names = F)
...
pdf("Figures/TregTconv3DS.pdf", useDingbats=FALSE, 8.27, 11.69) # width and height in inches.
g
dev.off()

#write.table(mat3, "OutputTables/HeatMapProtExp_Table.txt", sep = "\t", row.names = F)
```

The table used for the final heatmap is: `OutputTables/HeatMapProtExp_Table.txt`.
